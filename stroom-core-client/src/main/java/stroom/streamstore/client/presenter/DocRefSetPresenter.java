...
import stroom.process.shared.LoadDocRefSetAction;
...
public class DocRefSetPresenter extends MyPresenterWidget<DocRefSetView>
        implements DocRefSetUiHandlers {
    ...
    @Inject
    public DocRefSetPresenter(final EventBus eventBus, final DocRefSetView view,
    ...
    public <T extends BaseEntity> void read(final String type, final boolean groupedEntity,
                                            final DocRefSet entityIdSet) {
        this.groupedEntity = groupedEntity;

        if (entityIdSet != null && entityIdSet.getSet() != null) {
            // Load the entities.
            final SetId key = new SetId(type, type);
            final SharedMap<SetId, DocRefSet> loadMap = new SharedMap<>();
            loadMap.put(key, entityIdSet);
            final LoadDocRefSetAction action = new LoadDocRefSetAction(loadMap);
    ...
    public void write(final DocRefSet entityIdSet) {
        entityIdSet.clear();
        for (final DocRef docRef : data) {
            if (docRef != null) {
                entityIdSet.add(docRef);
            }
        }
    }
    ...
    public interface DocRefSetView extends View, HasUiHandlers<DocRefSetUiHandlers> {
